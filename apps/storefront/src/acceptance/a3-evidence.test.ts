...
describe('A3 · anti-fabrication', () => {
	...
	it('A3.4 every evidence reference resolves to a registry item', () => {
		...
		// Threshold is 7: retired profile and credential rows shrank the surface
		// that legitimately cites evidence.
	...
	it('A3.6 no employer is named and no entity suffix appears on a marketing surface', () => {
		...
		// Retired rows stay retired.
		expect(BY_ID.has('day-job-global-automation')).toBe(false);
